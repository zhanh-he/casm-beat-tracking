...
\begin{keywords}
beat tracking, context-aware, semi-Markov decoder, post-processing, local periodicity
\end{keywords}
...
For post-processors, the key design question is how to balance temporal regularity against local activation evidence. A weak regularity constraint preserves the network's predictions but may leave missed or spurious beats uncorrected. An overly strong constraint can instead suppress valid peaks under expressive timing or rapid local tempo variation, which is common in classical music performances~\cite{Schreiber2020LocalTempo}.
...
\item \textbf{Good generalization.} A frozen deployment of CASM can handle both steady (GTZAN, common genres) and complex rhythms (SMC, deliberately challenging) across the MIREX2026 beat tracking benchmark test datasets.
