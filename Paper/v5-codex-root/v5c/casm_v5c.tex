...
\label{sec:intro}
Framewise neural activations do not by themselves define a coherent beat and
downbeat sequence. Direct peak picking respects those activations but may miss
weak beats, duplicate events, or change metrical level; structured decoding can
restore continuity, but an ill-matched prior may override correct local
evidence. This tension is most visible in expressive timing, uncommon tempi,
and passages with multiple plausible pulse levels
\cite{chiu2023localperiodicity,foscarin2024beat,ahn2026smcblindspot}.

Classical dynamic programming, probabilistic beat-position models, CRFs, and
comb filters all use sequence or periodicity structure after local evidence has
been estimated \cite{ellis2007beattracking,korzeniowski2014crf,fillon2015crf,
bock2015combfilter}. DBNs are the prevailing joint decoder: they search a
tempo--phase--meter state space whose support and transition behaviour are
specified globally \cite{krebs2015dbn,bock2016joint}. PLPDP instead estimates a
time-varying inter-beat interval and confidence from predominant local pulse,
conditioning both the target and weight of dynamic programming on local
evidence \cite{chiu2023localperiodicity}. Yet pulse strength does not directly
measure the separation between the selected period and a competing half- or
double-tempo explanation; differentiable PLP likewise represents competition
through a soft mixture rather than an explicit event-level margin
\cite{chiu2025dplp}.

Semi-Markov rhythm modelling itself is not new. The one-dimensional model of
\cite{heydari2022semimarkov} performs efficient causal inference with a
jump-back state process, and BeatNet/BeatNet+ use particle filtering for online
joint rhythm analysis \cite{heydari2021beatnet,heydari2024beatnetplus}. In
contrast, our setting is offline: a frozen tracker has already produced the
complete activation sequence, and the objective is a maximum-score path over
sparse candidate-to-candidate segments. This also differs from methods that
remove or learn the decoder by changing the predictor
\cite{chen2022postprocessingfree,foscarin2024beat,foscarin2026masked}.

We propose the Context-Aware Semi-Markov (CASM) decoder. CASM estimates a local
period and its top-versus-competitor margin from the activation sequence, then
uses them to set the target, strength, and tolerance of a segment-duration cost.
Clear evidence strengthens local regularity; ambiguous evidence relaxes it
toward the neural output. The decoder stays on activation-supported peaks, uses
a separate count-ratio fallback to Direct, and places downbeats on the selected
beat grid. One calibration-selected configuration is frozen across tracks,
datasets, and backbones, with no predictor retraining or per-track BPM and meter
input. Our contribution is a new ambiguity-conditioned offline decoding rule
that is both narrowly defined and empirically strong: it improves every
reported metric over Direct across three backbones and two datasets, reaches
state-of-the-art SMC performance, and shows substantially lower calibration-set
variation than DBN in our sensitivity analysis.

\section{Methodology}
\label{sec:method}
\vspace{-6mm}
\rev{\subsection{CASM Overview}}
\rev{Given frozen beat and downbeat activations, CASM retains beat maxima as
candidates and scores transitions between them. At each candidate, an estimated
period specifies the preferred transition duration, while a normalized margin
between the best and strongest alternative periods determines the duration
cost's stiffness. This yields a single response rule with input-dependent
operating points: decisive periodic evidence favours regular segments, whereas
competition reduces the influence of the duration model.}
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/p0.jpg}
\caption{Input-conditioned CASM behaviour on held-out Beat This activations
from SMC. A decisive local period supports weak activation maxima (left), while
period competition relaxes the duration cost and yields the Direct path here
(right). References are visualisation aids only.}
\label{fig:casm-examples}
\end{figure}
\rev{Figure~\ref{fig:casm-examples} shows the two intended regimes. CASM never
creates an event away from the retained candidates: it either uses reliable
periodicity to select weak evidence or, under ambiguity, lets local evidence
dominate. Low margin causes this soft relaxation; it does not itself trigger a
hard fallback. The latter is an independent count-ratio safeguard applied after
path decoding.}
\begin{figure}[t!]
\centering
