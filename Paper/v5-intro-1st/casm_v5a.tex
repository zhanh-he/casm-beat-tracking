...
\label{sec:intro}
Beat and downbeat times provide a temporal scaffold for music production,
structural analysis, synchronization, and interactive systems
\cite{chiu2023localperiodicity,foscarin2024beat}. Contemporary trackers commonly
map audio to framewise activations and then decode those curves into discrete
events. This final step remains consequential even as neural predictors improve:
direct peak picking preserves local evidence but can leave duplicated, missing,
or metrically inconsistent events, whereas an overly strong temporal model can
replace valid evidence with an ill-fitting rhythmic interpretation. The decoder
must therefore obtain sequence coherence without treating every recording as if
its timing evidence were equally certain.

Structured beat tracking has long balanced local observations against temporal
regularity. Dynamic programming uses an inter-beat preference
\cite{ellis2007beattracking}; probabilistic event models and resonating comb
filters provide alternative sequence and periodicity representations
\cite{korzeniowski2014crf,bock2015combfilter}. Dynamic Bayesian networks (DBNs)
became the prevalent joint beat/downbeat decoder by inferring a path through
tempo, phase, and meter states \cite{krebs2015dbn,bock2016joint}. Although that
path responds to the observations, its tempo support, meter inventory, and
transition law are configured globally. These assumptions work well when they
match the music, but can suppress useful activations for very slow,
expressively timed, or structurally unusual material
\cite{foscarin2024beat,ahn2026smcblindspot}.

PLPDP is the closest methodological predecessor to our work. It extracts a
time-varying inter-beat interval and confidence from predominant local pulse
(PLP), then uses them as the target and weight of a dense-frame dynamic-programming
penalty \cite{chiu2023localperiodicity}. It therefore establishes that a decoder's
temporal prior can be conditioned on local evidence. Its confidence measures the
support of the selected pulse, however, rather than how decisively that period
outranks a competing half- or double-tempo interpretation. Later work makes PLP
causal and controllable or differentiable through mixtures of candidate kernels
\cite{meier2024realtime,chiu2025dplp}; CASM consequently does not claim local
periodicity or ambiguity handling in general as new.

Other structured approaches occupy a different inference setting. BeatNet and
BeatNet+ combine causal neural activations with particle filtering for online
joint rhythm analysis \cite{heydari2021beatnet,heydari2024beatnetplus}, while the
one-dimensional semi-Markov state space of \cite{heydari2022semimarkov} performs
causal framewise inference with an adaptive jump-back mechanism. These works
show that semi-Markov rhythm tracking itself is not new. CASM instead performs
offline maximum-score decoding over sparse, activation-supported
candidate-to-candidate segments, so it can use both past and future context from
a frozen activation stream. Approaches that retrain the predictor, learn an
iterative decoder, or use extra user annotations address complementary deployment
settings \cite{chen2022postprocessingfree,foscarin2024beat,foscarin2026masked,
pinto2021userdriven}.

We propose Context-Aware Semi-Markov (CASM) decoding, a model-agnostic
post-processor for beat and downbeat activations from a frozen tracker. Like
PLPDP, it estimates a local beat period from the activation sequence; unlike
PLPDP, it explicitly measures the separation between the dominant period and
its strongest competitor and uses that margin to control both the strength and
tolerance of an event-to-event duration preference. Clear periodic evidence
therefore supports stronger regularization, whereas ambiguity returns control
gradually to the neural observations. A separate beat-count safeguard handles
implausibly sparse or dense paths, and a beat-synchronous meter decoder preserves
beat--downbeat consistency. Our contribution is a novel, effective, and
interpretable offline decoder that improves Direct in every reported F1, CMLt,
and AMLt comparison across three frozen backbones and two corpora, without
backbone retraining or per-track BPM or meter settings.
