...
\label{sec:intro}
Beats provide the temporal scaffold for music analysis, synchronization, and
interactive systems. Modern neural trackers usually represent that scaffold as
framewise beat and downbeat activations, which must still be converted into a
discrete event sequence. Direct peak picking preserves local evidence but can
duplicate or miss beats, switch metrical level, or place downbeats off the beat
grid. A structured decoder can repair these errors, yet it creates a central
trade-off: too little structure sacrifices continuity, while too much can
override correct evidence in expressive or rhythmically unusual music
\cite{chiu2023localperiodicity,foscarin2024beat}.

Sequence models have long addressed this trade-off. Dynamic programming
balances onset evidence against inter-beat regularity
\cite{ellis2007beattracking}; probabilistic beat-position models, CRFs, and
resonating comb filters provide related event and periodicity formulations
\cite{korzeniowski2014crf,fillon2015crf,bock2015combfilter}. The dominant joint
decoder is the DBN, which infers a tempo--phase--meter path under globally
specified tempo support, transition behaviour, and meters
\cite{krebs2015dbn,bock2016joint}. These assumptions promote coherence when
they match the recording but may require corpus-specific retuning when they do
not \cite{ahn2026smcblindspot}. Existing one-dimensional semi-Markov models and
BeatNet/BeatNet+ instead target efficient causal joint rhythm-state inference
\cite{heydari2022semimarkov,heydari2021beatnet,heydari2024beatnetplus}; CASM
addresses the different, offline setting in which the complete activation
sequence from a frozen tracker is available.

PLPDP is the closest methodological predecessor. It replaces a global tempo
target with a time-varying inter-beat interval derived from predominant local
pulse (PLP), and scales its dynamic-programming penalty with PLP confidence
\cite{chiu2023localperiodicity}. This establishes that a timing constraint can
be conditioned on local activation evidence. Subsequent PLP work has explored
causal operation and differentiable mixtures of period hypotheses
\cite{meier2024realtime,chiu2025dplp}. However, the absolute support of a
selected pulse does not directly state how decisively it exceeds a competing
half- or double-tempo interpretation. When two periods are similarly plausible,
confidently enforcing either one risks turning useful structure into error.

We propose the Context-Aware Semi-Markov (CASM) decoder, a model-agnostic
post-processor for frozen beat and downbeat activations. Like PLPDP, CASM
estimates a local period and its support from the activation sequence; unlike
PLPDP, it defines support as the separation between the dominant and strongest
competing periods and uses this margin to control both the strength and
tolerance of an event-to-event duration preference. CASM therefore enforces
local regularity when periodic evidence is decisive and becomes increasingly
activation-led when it is ambiguous. A separate event-count safeguard can
return Direct, and a beat-synchronous meter decoder keeps downbeats on the
selected beat grid, all without backbone retraining or per-track BPM and meter
settings.

Our contribution is a new, transparent, and effective offline decoder that
combines an ambiguity-conditioned sparse semi-Markov path with deterministic
safeguards: using one frozen configuration, CASM improves every reported F1,
CMLt, and AMLt comparison over Direct across three backbones and two datasets,
achieves state-of-the-art results on SMC, and is less sensitive than DBNs to the
...
\rev{CASM constructs a sparse path over activation-supported peaks and
estimates both local periodicity and its ambiguity. The local period centres a
segment-duration preference, while its margin over the strongest competing
period controls the preference's strength and tolerance. When one period is
clearly supported, CASM can recover weak but activation-supported beats that
Direct misses; when period hypotheses compete, the duration term relaxes and
the activation evidence dominates. This is a graded change in the path score,
not an ambiguity-triggered switch: the separate count safeguard returns Direct
only if the resulting path is implausibly sparse or dense.}
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/p0.jpg}
\caption{Two held-out SMC excerpts illustrating CASM on Beat This activations.
A well-separated local period recovers weak, activation-supported beats (left);
competing periods relax the duration constraint, producing the Direct path in
this example (right). Reference beats and IBI are shown only for interpretation
and are not decoder inputs.}
\label{fig:casm-examples}
\end{figure}
\rev{Figure~\ref{fig:casm-examples} previews these complementary operating
regimes before the formal construction below. The first dynamic program selects
beats on a sparse candidate graph; a second, beat-synchronous program places
downbeats on that selected grid. Both stages are deterministic, and all scalar
parameters are calibrated once and then frozen.}
\begin{figure}[t!]
\centering
