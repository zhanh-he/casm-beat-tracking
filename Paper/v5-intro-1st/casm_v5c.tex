...
\label{sec:intro}
Every beat decoder makes a trust decision. Direct peak picking trusts the local
neural activations, preserving their detail but offering little protection
against missing, duplicated, or metrically inconsistent events. A strongly
structured decoder trusts rhythmic continuity, correcting local errors but also
risking the rejection of valid evidence. This trade-off matters because beat and
downbeat times form the temporal reference for transcription, structural
analysis, synchronization, and interactive music systems
\cite{chiu2023localperiodicity,foscarin2024beat}. The central question is not
whether to impose musical structure, but when the input has made that structure
reliable enough to override a local prediction.

Classical dynamic programming formalized this balance with an inter-beat
regularity term \cite{ellis2007beattracking}; probabilistic event models and
comb-filter banks supplied other sequence and periodicity representations
\cite{korzeniowski2014crf,bock2015combfilter}. DBNs became the standard joint
beat/downbeat solution by decoding an observation-dependent trajectory through
tempo, phase, and meter \cite{krebs2015dbn,bock2016joint}. Their state support,
meter set, and transition preferences are nevertheless specified globally. When
these assumptions mismatch slow, expressively timed, or structurally unusual
music, coherence can come at the cost of suppressing useful activations
\cite{foscarin2024beat,ahn2026smcblindspot}.

PLPDP changes the source of that commitment. It derives a time-varying
inter-beat interval and pulse-strength confidence from predominant local pulse
and uses them to condition the target and weight of dense-frame dynamic
programming \cite{chiu2023localperiodicity}. Thus, PLPDP already makes temporal
regularization local and confidence-aware. Yet confidence in a selected pulse is
not the same as its advantage over alternatives: half- and double-tempo periods
may both be strongly supported, making either one unsafe to enforce. Causal PLP
and differentiable mixtures of periodic kernels extend this local-periodicity
line in complementary directions \cite{meier2024realtime,chiu2025dplp}; our
claim is therefore not that local adaptation or ambiguity is new in isolation.

Nor is semi-Markov rhythm tracking itself new. The compact one-dimensional model
of \cite{heydari2022semimarkov} performs causal framewise inference with an
online-adapted jump-back distribution, while BeatNet and BeatNet+ pair causal
neural activations with particle filters for real-time joint rhythm analysis
\cite{heydari2021beatnet,heydari2024beatnetplus}. CASM addresses the complementary
offline plug-in setting: it receives activations from an already frozen tracker,
retains supported event candidates, and can use future as well as past context
when scoring complete candidate-to-candidate segments.

We propose Context-Aware Semi-Markov (CASM) decoding, in which rhythmic
commitment must be earned by local period competition. Around each candidate,
CASM compares the dominant period with its strongest competitor. A decisive
margin yields a strong, narrow duration preference; a small margin produces a
weak, broad preference, making the path increasingly observation-led. A separate
count safeguard handles implausibly sparse or dense results, and a
beat-synchronous meter decoder preserves beat--downbeat consistency. CASM's
central contribution is a novel and effective way to turn period competition
into an explicit trust signal for offline semi-Markov decoding: one frozen,
interpretable configuration improves Direct in every reported F1, CMLt, and
AMLt comparison across three backbones and two corpora, without retraining or
per-track BPM and meter settings.
