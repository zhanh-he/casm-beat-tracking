...
\label{sec:intro}
Beat tracking gives a music system a sense of where a performance is in time.
The pulse is often easy to follow when accents are clear and timing is stable,
but expressive timing, weak attacks, dense textures, and competing metrical
interpretations can make it locally uncertain. Neural trackers express that
uncertainty as framewise beat and downbeat activations. The final decoder must
then decide how much to trust each local peak and how much regularity to impose
on the sequence---a decision that can be as consequential as the activation
model itself \cite{chiu2023localperiodicity,foscarin2024beat}.

Direct peak picking takes the activations at face value. It avoids an
ill-matched tempo model, but can miss weak beats, duplicate events, or switch
metrical level. At the other extreme, dynamic programming and probabilistic
sequence models explicitly trade event evidence against temporal consistency
\cite{ellis2007beattracking,korzeniowski2014crf,fillon2015crf}. DBNs have become
the standard joint beat/downbeat decoder by searching tempo, phase, and meter
states under a predefined tempo range, transition law, and metrical inventory
\cite{krebs2015dbn,bock2016joint}. They are effective when these assumptions fit
the piece; outside that regime, the same structure can suppress correct local
evidence or motivate dataset-specific retuning
\cite{foscarin2024beat,ahn2026smcblindspot}.

PLPDP offers a more local answer. It derives a time-varying inter-beat interval
and a pulse-strength confidence from predominant local pulse (PLP), then uses
both to condition dense-frame dynamic programming
\cite{chiu2023localperiodicity}. Its key lesson is that the activation sequence
can determine the timing prior, rather than merely being scored by one.
Nevertheless, a strong selected pulse may coexist with a nearly as strong
half- or double-tempo alternative. Later PLP work treats causal operation or
soft mixtures of period kernels \cite{meier2024realtime,chiu2025dplp}, but a
frozen plug-in decoder still needs an explicit rule for how such competition
should alter an event path.

Related online systems solve a different problem. The one-dimensional
semi-Markov model of \cite{heydari2022semimarkov} compresses the rhythm state
space and adapts a jump-back distribution during causal inference, while
BeatNet and BeatNet+ maintain online rhythm hypotheses with particle filtering
\cite{heydari2021beatnet,heydari2024beatnetplus}. CASM neither claims the first
semi-Markov rhythm model nor targets real-time tracking: it uses future context
to compute an offline maximum-score path over activation-supported candidate
segments. Post-processing-free and learned iterative predictors make another
complementary choice by changing or retraining the activation model
\cite{chen2022postprocessingfree,foscarin2024beat,foscarin2026masked}; CASM
leaves that model untouched.

We introduce the Context-Aware Semi-Markov (CASM) decoder for this frozen-output
setting. It estimates a local period and the separation between the leading and
strongest competing period, then uses that margin to make its event-to-event
duration preference firm and narrow when the evidence is decisive, or weak and
broad when it is ambiguous. The path remains anchored to observed activation
maxima; a separate event-count safeguard can return Direct, and downbeats are
decoded only on the selected beat grid. All scalar parameters are selected once
and frozen, with no backbone retraining or per-track BPM and meter settings.

The contribution is therefore not semi-Markov modelling or local periodicity
in isolation, but a new ambiguity-conditioned offline decoder that combines
both in a transparent plug-in: one configuration improves all reported F1,
CMLt, and AMLt results over Direct across three backbones and two datasets,
sets the best reported SMC results, and transfers more stably than the DBN
configurations in our calibration study.

\section{Methodology}
\label{sec:method}
\vspace{-6mm}
\rev{\subsection{CASM Overview}}
\rev{CASM first reduces the beat activation sequence to a sparse set of local
maxima, so every output beat remains supported by the frozen tracker. It then
asks two questions at each candidate: which local period is strongest, and how
clearly does it defeat its nearest competitor? The first quantity locates the
preferred segment duration; the second decides how strongly and how precisely
that duration should influence the path. Clear evidence can bridge a weak
candidate without inventing a beat between peaks, whereas ambiguity makes the
search follow the activation evidence more closely.}
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/p0.jpg}
\caption{How local context changes CASM's behaviour on held-out Beat This/SMC
activations. Decisive periodicity supports a weak candidate missed by Direct
(left); competing periods weaken the duration preference and preserve Direct's
path in this example (right). Reference beats and IBI are displayed only for
interpretation.}
\label{fig:casm-examples}
\end{figure}
\rev{Figure~\ref{fig:casm-examples} makes this distinction concrete. On the
left, CASM restores activation-supported intermediate beats because the local
period is decisive. On the right, plausible competing periods flatten the
structural preference, so the selected events coincide with Direct. This local
relaxation is independent of the later beat-count safeguard: ambiguity changes
edge scores continuously, while only an implausible total count triggers an
exact return to Direct.}
\begin{figure}[t!]
\centering
